...
ztaun = ztau *(cos(2.*rpi*ztrel-rpi/4.)+3.)
ztaun = ztaun *(cos(2.*rpi*ztrel-rpi/4.)+3.)
DO jj = 1, jpj
DO ji = 1, jpi
!!!in this edition phimax = 45 phimin = 26
zyrel=( gphiu(ji,jj) - 26.) / (45.-26. )*2.3
utau(ji,jj) =   ztaun * SIN( 2*rpi *(zyrel/2.+0.5)+0.5*cos(2*rpi*ztrel-rpi/4.))
vtau(ji,jj) = 0.
END DO
END DO
...
!write(*,*) "rank = ",mpprank, " index= ", 2, jpim1
DO jj = 2, jpjm1
   ...
   SUBROUTINE usrdef_sbc_ice_flx( kt, phs, phi )
      ...
      DO jj = 1, jpj
         DO ji = 1, jpi
            ...
            qns_ice (ji,jj,jk) = - 40.e0  * ( tsb(ji,jj,1,jp_tem) + 1.9 ) - qsr(ji,jj)
