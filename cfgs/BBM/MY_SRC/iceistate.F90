MODULE iceistate
   ...
   REAL(wp) ::   rn_apd_ini_n, rn_hpd_ini_n, rn_dmg_ini_n
   REAL(wp) ::   rn_apd_ini_s, rn_hpd_ini_s, rn_dmg_ini_s
   !
   !                              ! if ln_iceini_file = T
   ...
   INTEGER , PARAMETER ::   jp_dmg = 10          ! index of ice damage       (-) !#bbm
...
CONTAINS
   ...
   SUBROUTINE ice_istate( kt )
      ...
      REAL(wp), DIMENSION(jpi,jpj)     ::   zapnd_ini, zhpnd_ini, zdmgt_ini            !data from namelist or nc file
...
!!      CALL dia_wri_state( 'output.init' )
   ...
   SUBROUTINE ice_istate_init
      ...
      TYPE(FLD_N)                    ::   sn_hti, sn_hts, sn_ati, sn_smi, sn_tmi, sn_tsu, sn_tms, sn_apd, sn_hpd, sn_dmg
      ...
      NAMELIST/namini/ ln_iceini, ln_iceini_file, rn_thres_sst, &
         ...
         &             sn_hti, sn_hts, sn_ati, sn_tsu, sn_tmi, sn_smi, sn_tms, sn_apd, sn_hpd, sn_dmg, cn_dir
      !!-----------------------------------------------------------------------------
      !
...
902   IF( ios >  0 )   CALL ctl_nam ( ios , 'namini in configuration namelist' )
      ...
      IF(lwp) THEN                          ! control print
         ...
         IF( ln_iceini .AND. .NOT.ln_iceini_file ) THEN
            ...
            WRITE(numout,*) '      initial damage in the north-south         rn_dmg_ini     = ', rn_dmg_ini_n,rn_dmg_ini_s
         ENDIF
      ENDIF
      !
